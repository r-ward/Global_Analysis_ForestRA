...
# Table S6: Candidate Forest Age schemes 
    # Table created in word for SI, see Table S7 code for mid ranges used
...
# Table S14: Full model site soil 
    ...
    # Create the table
    tab_model(full_RRL_site, full_R_site, full_L_site, 
            dv.labels = c("R/(R+L); field sampled soil", "R; field sampled soil", "L; field sampled soil"),
            show.aic = TRUE,
            # Save
            file = "Output/TableS14_SiteSoil_mod_table.docx")
...
# Fig S2: Correlation between denominators, log(R+L) and log(NPP)
    log_denom_plot <- get_denom_corr_plot(RNPP_sites)
...
# Fig S4: Box-cox transform plots
    ...
    rrl_bc_plots <- create_boxcox_comparison(df, "RRL", "RA proxy (R/(R+L))", "R/(R+L)","bc_RRL", "lambda")
    r_bc_plots <- create_boxcox_comparison(df, "Repro_flux_Mghayr", "reproductive litterfall (R)","R (Mg/hayr)", "bc_R", "lambda_r")
    l_bc_plots <- create_boxcox_comparison(df, "Leaf_flux_Mghayr", "leaf litterfall (L)", "L (Mg/hayr)", "bc_L", "lambda_l")
...
# Fig S5: Correlation plot 
    ...
    create_correlation_plot(d_mod, save_plot = TRUE)

# Fig S6-S8: Model diagnostic plots
    source("Functions/create_mod_diagnostic_plots.r")
    # Generate the diagnostic plots for each model - these take a minute to run. 
    ...
    # Save the plots as a .jpeg 
     ...
     ggsave(L_resid_grid, filename = "Output/FigureS8_L_residuals.jpeg", width = 8, height = 7, units = "in", dpi = 300)
