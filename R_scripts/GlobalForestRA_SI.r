...
# Tables S4: Full model summary table
    tab_model(mod_full_RRL, mod_full_R, mod_full_L, 
            dv.labels = c("R/(R+L)", "R (reproductive flux)", "L (leaf flux)"),
            show.aic = T) #,
            # Save
            # file = "Output/TableS4_FullMod_table.doc")

# Table S5: Final model summary table
    tab_model(mod_RRL, mod_R, mod_L, 
            dv.labels = c("R/(R+L)", "R (reproductive flux)", "L (leaf flux)"),
            show.aic = T) #,
            # Save
            # file = "Output/TableS5_FinalMod_table.doc")

# Table S6: Summary table from sensitivity analysis using sampling_duration as weight
    ...
    tab_model(mod_sqDur_RRL, mod_Dur_RRL, mod_noweight_RRL, 
            ...
            show.aic = T) #,
            # Save
            # file = "Output/TableS6_Mod_weighting_scheme_test.doc")

# Table S7: Candidate Forest Age schemes 
    # Table created in word, see Table S8 code for mid ranges used

# Table S8: Sensitivity of model fit to Forest Age scheme
...
# Table S9-12: ANOVA and Tukey's HSD biome; 
    source("Functions/create_word_tables.r")
    # Create Word documents for both analyses
    create_word_tables(df, 
                    group_var = "Biome",
                    doc_path = "Output/TableS9_S10_biome_statistics_tables.docx")

# Table S11-12: ANOVA and Tukey's HSD for biomd-plant type
    source("Functions/create_word_tables.r")
    create_word_tables(df_biome, 
                    group_var = "biome_leaftype",
                    doc_path = "Output/TableS11_S12pft_statistics_tables.docx")

# Table S13: Full model site soil 
...
# Fig S4: Box-cox transform plots
    ...
    rrl_bc_plots <- create_boxcox_comparison(df, "RRL", "R/(R+L)", "bc_RRL", "lambda")
    r_bc_plots <- create_boxcox_comparison(df, "Repro_flux_Mghayr", "R (reproductive flux)", "bc_R", "lambda_r")
    l_bc_plots <- create_boxcox_comparison(df, "Leaf_flux_Mghayr", "L (leaf flux)", "bc_L", "lambda_l")
